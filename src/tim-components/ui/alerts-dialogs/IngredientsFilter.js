...
const CategoryFilterDialog = (props) => {
    const { ingredientFacets, selectedFacets } = props
    const classes = useStyles();
    const searchInputClasses = useSearchInputStyles();
    const [open, setOpen] = React.useState(false);
    const [filterText, setFilterText] = useState('');
    const availableFacets = Object.keys(ingredientFacets)
        .map(ingredientKey => {
            const value = ingredientFacets[ingredientKey];
            return {
                name: ingredientKey,
                searchKey: `ingredients:${ingredientKey}`,
                value
            }
        })
...
const mapStateToProps = (state) => {
    const { ingredients } = state.availableFacets;
    const { selectedFacets } = state;

    return {
        ingredientFacets: ingredients,
        selectedFacets
    }
}
